...
public partial class ThreadedList<T> : IList<T>, IProducerConsumerCollection<T>, IReadOnlyThreadedList<T>
{
    ...
    /// <summary>
    /// Adds an item to the current <see cref="ThreadedList{T}"/>.
    /// </summary>
    ...
    /// <summary>
    /// Adds the items of a <see cref="IEnumerable{T}"/> to the end of the current <see cref="ThreadedList{T}"/>.
    /// </summary>
    /// <param name="collection"></param>
    ...
    /// <summary>
    /// Adds elements from a <see cref="ThreadedList{T}"/> to the end of the current <see cref="ThreadedList{T}"/>, from the given source index.
    /// </summary>
    /// <param name="source"></param>
    /// <param name="sourceIndex"></param>
    /// <param name="sourceCount"></param>
    public void AddRange(ThreadedList<T> source, int sourceIndex, int sourceCount)
    {
        _interlocker.Lock(() =>
        {
            EnsureCapacityInternal(_count + sourceCount);
            Array.Copy(source._items, sourceIndex, _items, _count, sourceCount);
            _count += sourceCount;
            _version++;
        });
    }
    ...
    /// <summary>Gets the number of items in the current <see cref="ThreadedList{T}"/>, without using any thread-safety.</summary>
    public int UnsafeCount => _count;

    /// <summary>
    /// Gets the number of items in the current <see cref="ThreadedList{T}"/>.
    /// </summary>
    public int Count
    {
        get
    ...
    /// <summary>
    /// Gets the Sync-root of the current <see cref="ThreadedList{T}"/>.
    /// </summary>
    object ICollection.SyncRoot => _locker;

    /// <summary>
    /// Gets the capacity of the current <see cref="ThreadedList{T}"/>.
    /// </summary>
    public int Capacity
    {
        get => _capacity;
