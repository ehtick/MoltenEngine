...
public class GpuFrameBuffer<T> : GpuObject
    where T : GpuObject
{
    public delegate T CreateResourceCallback(GpuDevice device);
    ...
    public GpuFrameBuffer(GpuDevice device, CreateResourceCallback createCallback) : 
        base(device)
    {
    ...
    /// <summary>
    /// Prepares the current frame-buffered resource and returns the correct <see cref="GpuResource"/> instance to use.
    /// </summary>
    /// <returns></returns>
    public T Prepare()
    {
